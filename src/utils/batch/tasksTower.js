...
export function createTasksTower(deps) {
  ...
  /**
   * 计算换皮闯关活动 actId = "26"+本周五月日(MMDD)+期号1
   */
  ...
  const skinChallenge = async () => {
    ...
    const taskPromises = selectedTokens.value.map(async (tokenId) => {
      ...
      try {
        ...
        // === 打完塔后自动抽奖 ===
        // 抽奖=走格子(activity_startactegame)，每步消耗道具5275(抽奖次数)；响应 role.items['5275'].quantity 是剩余次数。
        // 结束判据：剩余次数为0，或返回"物品不存在/不足"错误。响应cmd=activity_takeegamerewardresp(已在配对表补映射)。
        try {
          const lotteryActId = Math.floor(actIdNum / 10) * 10 + 2;
