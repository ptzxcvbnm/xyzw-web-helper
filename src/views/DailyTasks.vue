...
const refreshTasks = async () => {
  ...
  try {
    ...
    // 缓存到后端
    settingsApi.set(`dailyTasks_${selectedRoleId.value}`, mockTasks);
...
const executeTask = async (taskId) => {
  ...
  try {
    ...
    if (taskIndex !== -1) {
      ...
      // 保存到后端
      settingsApi.set(`dailyTasks_${selectedRoleId.value}`, tasks.value);
    }
...
onMounted(async () => {
  ...
  if (gameRolesStore.selectedRole) {
    ...
    // 尝试从后端加载任务数据
    const savedTasks = await settingsApi.get(
