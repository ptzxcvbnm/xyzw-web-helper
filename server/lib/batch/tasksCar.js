...
export function createTasksCar(deps) {
  ...
  const batchSmartSendCar = async () => {
    ...
    const taskPromises = selectedTokens.value.map(async (tokenId) => {
      ...
      if (!token) {
        addLog({ time: new Date().toLocaleTimeString(), message: `tokenId ${tokenId} 未找到，跳过`, type: "error" });
        return;
      }
  ...
  const batchClaimCars = async () => {
    ...
    const taskPromises = selectedTokens.value.map(async (tokenId) => {
      ...
      try {
        ...
        let claimedCount = 0;
        addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} [诊断] car_getrolecar 返回 ${carList.length} 辆车: ${carList.map(c => c.id).join(", ")}`, type: "info" });
        for (let car of carList) {
          if (shouldStop.value) break;
          const sendAtBefore = car?.sendAt;
          // car_getrolecar 对旧车只返回 null 占位(无 sendAt)，需用 car_detail 补全完整数据才能判断收取
          if (car && car.id && (car.sendAt === undefined || car.sendAt === null)) {
            try {
              const detail = await tokenStore.sendMessageWithPromise(
                tokenId,
                "car_detail",
                { carId: String(car.id), withLog: false, chat: false },
                10000,
              );
              const dc = detail?.car || detail?.body?.car || detail?._rawData?.car;
              if (dc && typeof dc === "object") car = { ...car, ...dc };
              addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} [诊断] 补详情 ${car.id}: sendAt ${sendAtBefore} -> ${car?.sendAt}, color=${car?.color}, detail键=${detail ? Object.keys(detail).join("|") : "空"}`, type: "info" });
            } catch (e) {
              addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} 补拉车辆详情失败(${car.id}): ${e.message}`, type: "warning" });
            }
          }
          const claimable = canClaim(car);
          const now = Date.now();
          const tsMs = Number(car?.sendAt || 0) < 1e12 ? Number(car?.sendAt || 0) * 1000 : Number(car?.sendAt || 0);
          addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} [诊断] ${car.id}: sendAt=${car?.sendAt} 已过=${car?.sendAt ? Math.floor((now - tsMs) / 60000) + "分钟" : "无"} canClaim=${claimable}`, type: "info" });
          if (claimable) {
            try {
              await tokenStore.sendMessageWithPromise(
                tokenId,
