...
export function createTasksTower(deps) {
  ...
  /**
   * 计算换皮闯关活动 actId
   * 规律: actId = "26" + 本周五月日(MMDD) + 期号
   * 活动每周五开始，整个周期 actId 不变（用本周五的日期）
   */
  const calcSkinActId = (phase = 1, date = new Date()) => {
    const d = new Date(date);
    const day = d.getDay(); // 0=周日, 5=周五
    // 回退到本周五（若今天是周五~周四，活动起始日是最近的过去的周五）
    let diff = (day - 5 + 7) % 7;
    d.setDate(d.getDate() - diff);
    const yy = String(d.getFullYear()).slice(2);
    const mm = String(d.getMonth() + 1).padStart(2, "0");
    const dd = String(d.getDate()).padStart(2, "0");
    return Number(`${yy}${mm}${dd}${phase}`);
  };
  ...
  const skinChallenge = async () => {
    ...
    const taskPromises = selectedTokens.value.map(async (tokenId) => {
      ...
      try {
        ...
        // 获取活动信息：换皮命令已变动，需带 actId（= 本周五日期 + 期号1）
        const skinActId = calcSkinActId();
        ...
        if (!towerData.actId) {
          try {
            const r0 = await tokenStore.sendMessageWithPromise(tokenId, "towers_getinfo", {}, 5000);
            const td0 = r0.actId ? r0 : (r0.towerData && r0.towerData.actId ? r0.towerData : r0);
            if (td0.actId) { res = r0; towerData = td0; }
          } catch {}
        }
        ...
        const actIdNum = towerData.actId;
        ...
        for (const type of targetTowers) {
            ...
            while (loop && !shouldStop.value) {
                if (needStart) {
                    await tokenStore.sendMessageWithPromise(tokenId, "towers_start", { towerType: type, actId: actIdNum }, 5000);
                    // 稍微等待一下
                    await new Promise(r => setTimeout(r, 500));
                }

                const fightRes = await tokenStore.sendMessageWithPromise(tokenId, "towers_fight", { towerType: type, actId: actIdNum }, 5000);
                ...
                if (curHP === 0) {
                     ...
                     // 刷新数据
                     res = await tokenStore.sendMessageWithPromise(tokenId, "towers_getinfo", { actId: actIdNum }, 5000);
        ...
        // === 打完塔后自动抽奖 ===
        // 抽奖=走格子(activity_startactegame)，每步消耗道具5275(抽奖次数)；响应里 role.items['5275'].quantity 是剩余次数。
        // 结束判据：剩余次数为0，或返回"物品不存在/不足"错误。抓包实证响应cmd=activity_takeegamerewardresp(已在配对表补映射)。
        try {
          const lotteryActId = Math.floor(actIdNum / 10) * 10 + 2; // 塔actId末位1 -> 抽奖末位2
          addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} 开始换皮抽奖`, type: "info" });
          let drawCount = 0;
          let timeoutStreak = 0;
          const maxDraw = 300; // 安全上限，防止死循环
          const readLeft = (r) => { try { const q = r?.role?.items?.["5275"]?.quantity; return typeof q === "number" ? q : null; } catch { return null; } };
          const isDoneErr = (m) => typeof m === "string" && (m.includes("物品不存在") || m.includes("次数") || m.includes("用尽") || m.includes("不足") || m.includes("上限"));
          while (drawCount < maxDraw && timeoutStreak < 5 && !shouldStop.value) {
            try {
              const r = await tokenStore.sendMessageWithPromise(tokenId, "activity_startactegame", { actId: lotteryActId }, 5000);
              drawCount++;
              timeoutStreak = 0;
              const left = readLeft(r);
              if (left === 0) { addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} 抽奖次数已用尽(共抽${drawCount}次)`, type: "info" }); break; }
              await new Promise(r => setTimeout(r, 350));
            } catch (e) {
              const m = e.message || "";
              if (isDoneErr(m)) { addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} 抽奖次数用尽`, type: "info" }); break; }
              if (m.includes("超时")) { timeoutStreak++; } else { addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} 抽奖异常(继续): ${m}`, type: "warning" }); }
              await new Promise(r => setTimeout(r, 800));
            }
          }
          // 循环领取累计奖励：每次领一档并返回 nextStageId，直到没有下一档或报错
          let claimStages = 0;
          while (claimStages < 50 && !shouldStop.value) {
            try {
              const cr = await tokenStore.sendMessageWithPromise(tokenId, "activity_actegamestageclaim", { actId: lotteryActId }, 5000);
              claimStages++;
              if (!cr || cr.nextStageId === undefined || cr.nextStageId === null) break;
            } catch (e) { break; }
            await new Promise(r => setTimeout(r, 300));
          }
          addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} 换皮抽奖完成，共抽 ${drawCount} 次，领取累计奖励 ${claimStages} 档`, type: "success" });
        } catch (e) {
          addLog({ time: new Date().toLocaleTimeString(), message: `${token.name} 换皮抽奖失败: ${e.message}`, type: "warning" });
        }
