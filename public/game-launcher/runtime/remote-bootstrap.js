...
window.convertAssets = function (url) {
  ...
  const runtimeConfig = window.__AUDIT_RUNTIME_CONFIG__ || {}
  const resourceOrigin = runtimeConfig.resourceOrigin || 'https://xxz-xyzw-res.hortorgames.com'
  let newUrl = resourceOrigin.replace(/\/$/, '') + '/remote/' + url.slice(7)
...
window.loadJscAndDecode = async function (url, callback) {
  ...
  const runtimeConfig = window.__AUDIT_RUNTIME_CONFIG__ || {}
  const encryptedScriptKey = runtimeConfig.encryptedScriptKey
  if (!encryptedScriptKey) {
    throw new Error('encrypted script key is missing from runtime config')
  }
  const jsCodeData = xxtea.decrypt(uint8Data, xxtea.toBytes(encryptedScriptKey))
  ...
  if (url.indexOf('/remote/launcher/') !== -1) {
    const clipboardBridgeCall = 'i.setClipboard('
    const clipboardBridgeCallCount = jsCode.split(clipboardBridgeCall).length - 1
    if (clipboardBridgeCallCount > 1) {
      throw new Error('launcher clipboard bridge matched more than once')
    }
    if (clipboardBridgeCallCount === 1) {
      if (
        !window.AuditedClipboard ||
        typeof window.AuditedClipboard.setClipboard !== 'function'
      ) {
        throw new Error('audited clipboard adapter is unavailable')
      }
      jsCode = jsCode.replace(
        clipboardBridgeCall,
        'window.AuditedClipboard.setClipboard('
      )
      console.log('[audit] redirected launcher clipboard bridge')
    }
  }
...
window.loadRemoteBundleVers = async function () {
  const runtimeConfig = window.__AUDIT_RUNTIME_CONFIG__ || {}
  const manifestUrl = runtimeConfig.manifestUrl
  if (!manifestUrl) {
    throw new Error('manifest URL is missing from runtime config')
  }
...
window.installRemoteAssetLoader = function () {
  ...
  const runtimeConfig = window.__AUDIT_RUNTIME_CONFIG__ || {}
  const resourceOrigin = runtimeConfig.resourceOrigin || 'https://xxz-xyzw-res.hortorgames.com'
  const escapedResourceOrigin = resourceOrigin.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  const encryptedBundleRE = new RegExp(
    '^' + escapedResourceOrigin + '/remote/(?:game|launcher|TEST_REMOTE_MODULE)(?:/|$)'
  )
  ...
  function executeScript(code, url, onComplete) {
    ...
    script.onerror = function () {
      script.parentNode && script.parentNode.removeChild(script)
      URL.revokeObjectURL(blobUrl)
      onComplete && onComplete(new Error('decrypted script execution failed: ' + url))
    }
    ...
    if (encryptedBundleRE.test(finalUrl) && finalUrl.endsWith('.jsc')) {
      window
        .loadJscAndDecode(finalUrl, function (code) {
          executeScript(code, finalUrl, function (err) {
            if (!err) loadedScripts[finalUrl] = true
...
window.boot = async function () {
  function markBootStage(stage) {
    window.__AUDIT_BOOT_STAGE__ = stage
    var statusNode = document.getElementById('audit-status')
    if (statusNode) statusNode.dataset.bootStage = stage
  }

  markBootStage('manifest:loading')
  try {
  ...
  markBootStage('manifest:ready')

  window.installRemoteAssetLoader()
  markBootStage('asset-loader:ready')

  var settings = window._CCSettings
  window._CCSettings = undefined
  var resolveBoot
  var rejectBoot
  var bootCompletion = new Promise(function (resolve, reject) {
    resolveBoot = resolve
    rejectBoot = reject
  })

  function failBoot(stage, error) {
    markBootStage('failed:' + stage)
    var cause = error instanceof Error ? error : new Error(String(error || 'unknown error'))
    cause.message = stage + ': ' + cause.message
    console.error('[audit] ' + cause.message, cause.stack)
    rejectBoot(cause)
  }
  ...
  function isIosMultiInstanceFrame() {
    ...
    var isIosDevice =
      (cc.sys.isBrowser && cc.sys.os === cc.sys.OS_IOS) ||
      /iPad|iPhone|iPod/.test(userAgent) ||
      (platform === 'MacIntel' && Number(navigatorObject.maxTouchPoints) > 1)
    return isIosDevice && /(?:^|[?&])slot=[1-4](?:&|$)/.test(window.location.search)
  }

  var onStart = function () {
    markBootStage('cocos:on-start')
    var iosMultiInstance = isIosMultiInstanceFrame()
    cc.view.enableRetina(!iosMultiInstance)
    ...
    // Reduce transient decoding and allocation pressure on Android and iOS multi-instance frames.
    // This must be configured before the scene starts loading.
    if (iosMultiInstance || (cc.sys.isBrowser && cc.sys.os === cc.sys.OS_ANDROID)) {
      cc.assetManager.downloader.maxConcurrency = 2
      cc.assetManager.downloader.maxRequestsPerFrame = 2
    }

    if (iosMultiInstance && document.documentElement) {
      document.documentElement.dataset.instanceMemoryMode = 'ios-low'
      console.log('[audit] enabled iOS multi-instance low-memory mode')
    }
    ...
    if (!bundle) {
      failBoot('locate launch scene', new Error(launchScene + ' is not present in loaded bundles'))
      return
    }

    markBootStage('scene:loading')
    bundle.loadScene(launchScene, null, null, function (err, scene) {
      if (err) {
        failBoot('load launch scene', err)
        return
      }

      cc.director.runSceneImmediate(scene)
      markBootStage('scene:running')
      if (cc.sys.isBrowser) {
        // show canvas
  ...
  function cb(err) {
    if (err) {
      failBoot('load startup dependency', err)
      return
    }
    count++
    markBootStage('startup-dependency:' + count + '/' + (bundleRoot.length + 1))
    if (count === bundleRoot.length + 1) {
      markBootStage('main-bundle:loading')
      cc.assetManager.loadBundle(MAIN, function (err) {
        if (err) {
          failBoot('load main bundle', err)
          return
        }
        if (
          window.AuditedAccountLogin &&
          typeof window.AuditedAccountLogin.installHooks === 'function'
        ) {
          window.AuditedAccountLogin.installHooks()
        }
        markBootStage('cocos:run')
        cc.game.run(option, onStart)
      })
    }
  }

  var runtimeConfig = window.__AUDIT_RUNTIME_CONFIG__ || {}
  var ignoredStartupScripts = runtimeConfig.ignoredStartupScripts || []
  var startupScripts = settings.jsList
    .filter(function (x) {
      var ignored = ignoredStartupScripts.indexOf(x) >= 0
      if (ignored) {
        console.warn('[audit] skipped startup script; local compatibility shim is active:', x)
      }
      return !ignored
    })
    .map(function (x) {
      // Some recovered settings entries are already rooted at assets/.
      // Prefixing those with src/ bypasses convertAssets() and causes a
      // local /src/assets/... 404 instead of loading the encrypted CDN file.
      if (/^(?:https?:\/\/|assets\/|\.{1,2}\/)/.test(x)) {
        return x
      }
      return 'src/' + x
    })
  ...
  if (startupScripts.length) {
    markBootStage('startup-scripts:loading')
    cc.assetManager.loadScript(startupScripts, cb)
  } else {
    cb()
  }

  for (var i = 0; i < bundleRoot.length; i++) {
    markBootStage('root-bundles:loading')
